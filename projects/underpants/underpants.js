...
_.partition = function(collection, test){
    const parted = [];
    parted[0] = _.filter(collection, function(value, position, collection){
        return (test(value, position, collection)); 
    });
    parted[1] = _.reject(collection, function(value, position, collection){
        return (test(value, position, collection));
    });
    return parted;
};
...
_.unique = function(collection){
    const uniqs = [];
    _.each(collection, function(value, position, collection){
        if(_.indexOf(uniqs, value) === -1) uniqs.push(value);
    });
    return uniqs;
};
...
_.map = function(collection, funct){
    const mapd = [];
    _.each(collection, function(value, position, collection){
        mapd.push(funct(value, position, collection));
    });
    return mapd;
};
...
_.pluck = function(collection, prop){
    const propArr = [];
    _.map(collection, function(value, position, collection){
        propArr.push(collection[position][prop]);
    });
    return propArr;
};
...
_.every = function(collection, func){
    ...
    if(typeof func !== 'function'){
        _.each(collection, function(value, position, collection){
        if(!value) allOfEm = false;
        });
    }
    else {_.each(collection, function(value, position, collection){
        if(func(value, position, collection) === false) allOfEm = false;
    });
    }
    return allOfEm;
};
...
_.some = function(collection, func){
    var allOfEm = false;
        if(typeof func !== 'function'){
            _.each(collection, function(value, position, collection){
            if(value) allOfEm = true;
            });
        }
        else {_.each(collection, function(value, position, collection){
            if(func(value, position, collection) === true) allOfEm = true;
        });
        }
        return allOfEm;
};
...
_.extend = function(object, object2){
    _.each(object2, function(value, position, collection){
